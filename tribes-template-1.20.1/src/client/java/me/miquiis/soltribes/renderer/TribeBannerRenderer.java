...
public class TribeBannerRenderer implements RenderRegionEvent.Text {
    ...
    private void renderBanner(MinecraftClient client, DrawContext context, Tribe tribe, int x, int y, int size) {
        if (tribe != null) {
            if (!tribe.getTribeBanner().isEmpty()) {
                ...
                this.renderBanner(client, context.getMatrices(), patterns, x, y, size);
            }
        }
    }

    private void renderBanner(MinecraftClient client, MatrixStack matrices, List<Pair<RegistryEntry<BannerPattern>, DyeColor>> patterns, int x, int y, int size) {
